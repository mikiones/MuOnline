...
public class CopyOnWriteArrayList implements List, Cloneable,
	 ...
	 */
	private static final long serialVersionUID = 3763093076863955507L;
	/**
	...
	public synchronized Object set(int index, Object element) {
		final int len = array_.length;
		rangeCheck(index, len);
		final Object oldValue = array_[index];

		final boolean same = (oldValue == element || (element != null && element
				.equals(oldValue)));
		if (!same) {
	...
	public synchronized boolean removeAll(Collection c) {
		final Object[] elementData = array_;
		final int len = elementData.length;
		if (len == 0) {
			return false;
		}
	 ...
	 * iterator. The iterator does <em>NOT</em> support the <code>remove</code>
	 * method.
	 */
	@Override
	 ...
	 * useful for CopyOnWriteArrays. </p> The semantics of the List returned by
	 * this method become undefined if the backing list (i.e., this List) is
	 * <i>structurally modified </i> in any way other than via the returned
	 * List. (Structural modifications are those that change the size of the
	...
	protected static class COWSubList extends AbstractList {
		...
		public Object set(int index, Object element) {
			synchronized (l) {
				...
				final Object x = l.set(index + offset, element);
